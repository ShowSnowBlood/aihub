...
function githubTargetFromUrl(value?: string | null): GithubTarget | null {
...
function extractSkillSummary(markdown: string) {
...
function isValidSkillMarkdown(markdown: string, summary: string) {
...
async function updateRow(row: SkillRow, result: MarkdownResult) {
  ...
  const weakDescription =
    !currentDescription ||
    slugToken(currentDescription) === slugToken(row.name) ||
    /^external-/i.test(currentDescription) ||
    /\u81ea\u52a8\u6c49\u5316\u6458\u8981|\u516c\u5f00\s+Skill|skills\.sh/i.test(currentDescription)
...
async function main() {
  ...
  let updated = 0
  let failed = 0
  const samples: Array<{ id: number; name: string; sourceUrl?: string | null; summary?: string; error?: string }> = []
